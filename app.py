...
with tab2:
    ...
    if st.session_state['streak'] >= 3:
        st.error(f"🚨 แจ้งเตือนด่วน: พบปัสสาวะมีลักษณะผิดปกติติดต่อกัน {st.session_state['streak']} วัน!")
        st.write("การมีฟองในปัสสาวะหรือสีผิดปกติสะสมเกิน 3 วันติดต่อกัน อาจส่งผลถึงภาวะโปรตีนรั่วในปัสสาวะ (Proteinuria) หรือไตทำงานผิดปกติ")
    else:
        st.metric("จำนวนวันที่พบความผิดปกติสะสม (Streak)", f"{st.session_state['streak']} วัน")
